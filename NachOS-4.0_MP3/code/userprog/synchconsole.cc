...
//      Write a converted integer string to the console display, waiting if necessary.
//----------------------------------------------------------------------

void
SynchConsoleOutput::PutInt(char *converted_int, int size)
{
    lock->Acquire();
    consoleOutput->PutInt(converted_int, size);
    waitFor->P();
    lock->Release();
}
